...
interface Equipment { id: number; name: string; category: string; unique_number: string | null; daily_rate: number; }
interface ActiveRental { id: number; invoice_number: string; customer_name: string; start_date: string; issue_time: string; total_amount: number; status: string; }

interface CartItem { equipment: Equipment; days: number; qty: number; subtotal: number; }
...
export default function Rentals() {
  ...
  const fetchData = async () => {
    try {
      ...
      const rentalResult = await db.select<ActiveRental[]>(`
        ...
        FROM rentals r JOIN customers c ON r.customer_id = c.id WHERE r.status = 'Issued' ORDER BY r.id DESC
      `);
  ...
  const selectedItemDetails = equipmentList.find(eq => eq.id.toString() === selectedEqId);
  const isBulkItem = selectedItemDetails?.category === 'Bulk Items';

  const handleAddToCart = () => {
    if (!selectedItemDetails) return;

    if (cart.some(c => c.equipment.id === selectedItemDetails.id)) {
    ...
    const qtyNum = isBulkItem ? (parseInt(rentalQty) || 1) : 1;
    
    const subtotal = selectedItemDetails.daily_rate * days * qtyNum;

    setCart([...cart, { equipment: selectedItemDetails, days, qty: qtyNum, subtotal }]);
    setSelectedEqId('');
    setRentalDays('1');
    setRentalQty('1'); 
  };
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!customerName || !customerNic) return setModal({ isOpen: true, title: 'Missing Info', message: 'Please fill in Customer Name and NIC.', type: 'error' });
    if (cart.length === 0) return setModal({ isOpen: true, title: 'Empty Invoice', message: 'Please add at least one item.', type: 'error' });
    ...
    try {
      ...
      const custResult = await db.execute("INSERT INTO customers (name, phone, nic, nic_photo_path) VALUES ($1, $2, $3, $4)", [customerName, customerPhone, customerNic, nicPhotoPath]);
      const rentResult = await db.execute("INSERT INTO rentals (customer_id, start_date, issue_time, expected_return, total_amount, invoice_number, status) VALUES ($1, $2, $3, $4, $5, $6, 'Issued')", [custResult.lastInsertId, issueDate, issueTime, issueDate, cartTotal, invoiceNumber]);

      for (const cartItem of cart) {
        await db.execute(
          "INSERT INTO rental_items (rental_id, equipment_id, qty, daily_rate) VALUES ($1, $2, $3, $4)",
          [rentResult.lastInsertId, cartItem.equipment.id, cartItem.qty, cartItem.equipment.daily_rate]
        );
        
        if (cartItem.equipment.category !== 'Bulk Items') {
          await db.execute("UPDATE equipment SET status = 'Rented' WHERE id = $1", [cartItem.equipment.id]);
        }
      }

      const pdfItems = cart.map(c => ({ ...c.equipment, qty: c.qty }));

      generateInvoicePDF({
        invoice_number: invoiceNumber, customer_name: customerName, nic: customerNic,
        date: issueDate, time: issueTime, items: pdfItems, days: cart[0]?.days || 1, total: cartTotal
      }, 'Issue');

      setCustomerName(''); setCustomerPhone(''); setCustomerNic(''); setNicPhotoPath(''); setCart([]);
      fetchData();
      setModal({ isOpen: true, title: 'Invoice Created!', message: `Invoice ${invoiceNumber} has been issued successfully. The PDF is downloading.`, type: 'success' });
    } catch (error) { setModal({ isOpen: true, title: 'Save Failed', message: String(error), type: 'error' }); }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-end gap-4">
                ...
                {isBulkItem && (
                  <div className="w-1/4 animate-fade-in">
                    <label className="block text-sm font-medium text-blue-600 dark:text-blue-400 mb-1">Quantity</label>
                    <input type="number" min="1" value={rentalQty} onChange={(e) => setRentalQty(e.target.value)} className="w-full bg-blue-50 dark:bg-blue-900/20 dark:text-white border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40 transition-colors" />
                  </div>
                )}

                {/* UPDATED: h-[46px] changed to h-11.5 */}
                <button type="button" onClick={handleAddToCart} className="flex-1 bg-gray-100 dark:bg-gray-800 text-dreamco-dark dark:text-gray-200 border border-gray-200 dark:border-gray-700 px-6 py-2.5 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-all font-medium h-11.5">
                  Add to List &darr;
                </button>
              </div>
            </div>
          ...
          <div className="pt-6 border-t border-gray-100 dark:border-gray-800 transition-colors">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden mb-4 transition-colors">
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {cart.length === 0 ? (
                    <tr><td colSpan={6} className="py-6 text-center text-gray-400 dark:text-gray-500 text-sm">No items added to this invoice yet.</td></tr>
                  ) : (
                    cart.map((c, index) => (
                      <tr key={index} className="border-b border-gray-50 dark:border-gray-800/50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-blue-600 dark:text-blue-400 font-bold">{c.qty}</td>
                        <td className="py-3 px-4 text-gray-800 dark:text-gray-200 font-medium">Rs. {c.subtotal.toLocaleString()}</td>
      ...
      <div className="bg-white dark:bg-gray-900/60 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden mt-8 transition-colors">
        ...
        {/* UPDATED: min-w-[600px] changed to min-w-150 */}
