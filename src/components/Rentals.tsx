...
export default function Rentals() {
  ...
  const fetchData = async () => {
    try {
      ...
      window.alert("Database Load Error: " + error);
    }
  };
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.alert("Database Save Error: " + error);
    }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" required value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">NIC *</label>
                  <input type="text" required value={customerNic} onChange={(e) => setCustomerNic(e.target.value)} placeholder="e.g. 199012345678" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">Phone</label>
                  <input type="text" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={selectedCategory} onChange={(e) => { setSelectedCategory(e.target.value); setSelectedEqId(''); }} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40">
                ...
                <div>
                  ...
                  <select value={selectedEqId} onChange={(e) => setSelectedEqId(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40">
              ...
              <div className="flex items-end gap-4">
                <div className="w-1/3">
                  ...
                  <input type="number" min="1" value={rentalDays} onChange={(e) => setRentalDays(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-dreamco-blue/40" />
                </div>
                  ...
                  {cart.length === 0 ? (
                    ...
                    cart.map((c, index) => (
                      <tr key={c.equipment.id} className="border-b border-gray-50">
                        <td className="py-3 px-4 font-medium text-gray-800">{c.equipment.name}</td>
                        <td className="py-3 px-4 text-gray-600">Rs. {c.equipment.daily_rate}</td>
                        <td className="py-3 px-4 text-gray-600">{c.days}</td>
                        <td className="py-3 px-4 text-gray-800 font-medium">Rs. {c.subtotal}</td>
            ...
            {activeRentals.length === 0 ? (
              ...
              activeRentals.map((rental) => (
                <tr key={rental.id} className="border-b border-gray-50">
                  ...
                  <td className="py-4 px-6 text-gray-500 text-sm">{rental.start_date} <br/><span className="text-gray-400">{rental.issue_time}</span></td>
